...
def test_mysql_foreign_key_set_null_parses_multiple_tables():
    ddl = """
    ...
    CREATE TABLE `student` (
      ...
      CONSTRAINT `student_ibfk_1` FOREIGN KEY (`dept_name`)
    ...
    result = DDLParser(ddl).run(output_mode="mysql")
    assert len(result) == 2
    assert result[1]["constraints"]["references"][0]["on_delete"] == "SET NULL"
