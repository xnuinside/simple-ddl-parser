...
### Fixed
- MySQL dump-style DDL files now parse more reliably: `DROP TABLE IF EXISTS` preambles no longer fail in strict mode, inline `/*...*/` comment placeholders inside identifiers are handled, and table-level `KEY` / `UNIQUE KEY` definitions with prefix lengths like `column(32)` are supported. https://github.com/xnuinside/simple-ddl-parser/issues/148
