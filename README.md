...
- CREATE [OR REPLACE] TABLE [ IF NOT EXISTS ] + columns definition, columns attributes: column name + type + type size(for example, varchar(255)), UNIQUE, PRIMARY KEY, DEFAULT, CHECK, NULL/NOT NULL, REFERENCES, ON DELETE, ON UPDATE, NOT DEFERRABLE, DEFERRABLE INITIALLY, GENERATED ALWAYS / GENERATED BY DEFAULT AS IDENTITY, STORED, COLLATE, inline `-- comments`
...
- CREATE TABLE ... AS SELECT ... FROM source_table statement, when the source table is defined in the same DDL script. Supported for a single source table with `SELECT *` or an explicit column list; the new table gets copied column definitions for the selected columns only.
...
### Example: CREATE TABLE AS SELECT from the same DDL

The parser can build a table definition from another table only when the referenced source table is already present in the same DDL input.
...
In this case `TestTable` will be returned as a normal parsed table with copied column definitions for `person_id` and `first_name`.
...
- PostgreSQL parsing now supports `GENERATED BY DEFAULT AS IDENTITY(...)` and complex `CONSTRAINT ... CHECK` expressions in table definitions. https://github.com/xnuinside/simple-ddl-parser/issues/268 (Fix #268)
